...
// The entity manager stores all components and entities internally
// It's like a giant memory database
...
boc.core.EntityManager = function () {
    ...
    this.addComponentToEntity = function (componentObj, entityId) {
        ...
        if (!entArr) {
            ...
            for (var i = 0; i < entArr.length; i++) {
                if (entArr === entityId) {
                    throw 'Entity already has component ' + className;
                }
            }
        }
    ...
    this.removeComponentFromEntity = function (componentName, entityId) {
        ...
        //    }
        //}
        for (var i = 0, entArrLen = entArr.length; i < entArrLen; i++) {
            if (entityId === entArr[i]) {
                entIndex = i;
                break;
            }
        }
        if (entIndex === -1) {
            return; /*throw 'Entity does not have component ' + componentName;*/
        }
        ...
        var compObjIndex = -1;
        //i = 0;
        //while (a = compObjArr[i++]) {
        //    if (compObj === a) {
        //        compObjIndex = i - 1;
        //        break;
        //    }
        //}
        for (var i = 0, compObjArrLen = compObjArr.length; i < compObjArrLen; i++) {
            if (compObj === compObjArr[i]) {
                compObjIndex = i;
                break;
            }
        }
        //compObjArr.splice(compObjArr.indexOf(compObj), 1);
        compObjArr.splice(compObjIndex, 1);

        //var entIndex = entArr.indexOf(entityId);
        entArr.splice(entIndex, 1);        

        //delete compHash[componentName];        
        compHash[componentName] = null;
    ...
    this.killEntity = function (entityId) {        
        ...
        _entitiesToComponent[entityId] = null;
        _entitiesToListeners[entityId] = null;
    ...
    window.$em = function (c) {
        ...
        var self = {
            ...
            add: function (c, params) {
                if (entity) {
                    if (typeof (c) == 'object' && c.className && typeof(c.className) === 'function'/*$.isFunction(c.className)*/) {
