...
bbq.Game = function (p)
{
    ...
    this.load = function (gameState) {
        camera.update({ zoom: 1.0 });
        ...
        if (gameState != null && typeof (gameState) != 'undefined') {
            ...
            panningCameraSystem.bounds(this.map().bounds());
            ...
            fogSystem = new bbq.systems.FogSystem(em, this.map());
            uiSystem = new boc.systems.UISystem(em);
            unitSystem = new bbq.systems.UnitSystem(em);
            commandSystem = new bbq.systems.CommandSystem(em, p.canvas, this.map());
            moveSystem = new bbq.systems.MoveSystem(em, this.map());
            attackSystem = new bbq.systems.AttackSystem(em, this.map());
            unitCollisionSystem = new bbq.systems.UnitCollisionSystem(em, this.map());
            hudSystem = new bbq.systems.HudSystem(em, this.hud());
            delayedMethodSystem = new boc.systems.DelayedMethodSystem(em);
            trainingSystem = new bbq.systems.TrainingSystem(em, renderSystem.camera(), this.map(), { scale : 1/camera.zoom } );
            healthSystem = new bbq.systems.HealthSystem(em);
            upkeepSystem = new bbq.systems.UpkeepSystem(em);
            saveSystem = new bbq.systems.SaveSystem(this, em);
            victorySystem = new bbq.systems.VictorySystem(this, em);
        }                
        // setup code    
        ...
        for (var playerid in p.gameState.state) {
            ...
            var unitSet = new bbq.UnitSet(pState.team, pState.unitSet || [ 'SpearWarrior', 'DartBlower', 'Scout', 'Enforcer', 'Gatherer', 'Catapult' ]); // default
            ...
            if (pState.units) {
                for (var i = 0; i < pState.units.length; i++) {
                    ...
                    //    if (_user && _user.playerid == this.getCurrentPlayer().id) {
                    //        boc.utils.createEvent(
                    ...
                    //    new bbq.commands.Gather().execute({ entity: unitEnt, map: this.map(), showIconOnly: true });
                    //}
                    //if (uState.options && uState.options.captureTarget) {
                    //    new bbq.commands.Capture().execute({ entity: unitEnt, map: this.map(), showIconOnly: true });
                    //}
                } // i
            }            
        } // each player
        ...
        turnEnt.addComponent(
            new bbq.components.Turn({
                ...
                number: this.getCurrentPlayer().turn
            })
        );
        ...
        if (userPlayer) {
            ...
            if (userPlayer.id == this.getCurrentPlayer().id) {
